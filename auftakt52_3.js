/*
【auftakt52_３.js】2025/06/11　auftakt52g.jsを元に開発
*/
/********** 本スクリプトの目的・成果 ***************
・これまでの検討成果を反映、本公開版を目指す
・色指定をスクリプトで　日本の伝統色　藍色ベースに　地面　地上　上空のイメージ？　ドットは翡翠
・Wake Lockは最初のスタート時必ず設定alertで知らせる。
	この段階で一区切りと思ったら、sdelayを０以外にすると、指標が見えなくなる。正規化時間を求めたあとに入れていた。→正規化時間を求めるところと、拍点検出の両方に入れた。
	やはり遅延を大きくするとば行分割音の音が欠ける

・開始までの待ち時間0,2.4秒を実装2025/06/11 22:46 OK
この時点でGITHUBに送る
...
→スタータスをチェックするコードを加え、メッセージエリアに表示し現状分析する。
	リリースのときに使っていたwakeLock→wakelockに変更してみた。
 	ONにした途端に、myCanvasでのスタートができなくなった。touchmoveは拾っている。
...
function chkWakeLock() {
	if (navigator.wakeLock) {
	  ...
	  if (isWakeLockActive) {
		...
		alert("Screen Wake Lock is NOT active.");
	  }
	} else {
		// Wake Lock APIがサポートされていない
		alert("Wake Lock API is not supported on this browser.");
		//設定メニューの該当部分を表示しない
