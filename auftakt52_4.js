...
setTempoも関数にするほどのことではない。
変数bpmも、一箇所でしか使われていない。
topMarginは二箇所で宣言、定義されている。一方で最高点を規定するところではこの変数が使われていない？
...
//
let cvMain;	//動指標が動くcanvas
let ctxMain;	//インスタンス
let cvBeat;  //拍子エリアのキャンバス
let ctxBeat;
...
//---DOM関連------------------
cvMain = document.getElementById("myCanvas");
ctxMain = cvMain.getContext("2d");

cvBeat = document.getElementById("beatCanvas");       //拍子表示キャンバス（画面下部）
ctxBeat = cvBeat.getContext("2d");
//背景色設定
...
function reserveSound() {
	//■次のサウンドの予約
	//console.log('■次のサウンドの予約beatTick = '+beatTick(MM * ndivBeat));
